...
const About = forwardRef<HTMLElement, AboutProps>(({ onStatClick }, ref) => {
    ...
    const stats = [
        {
            icon: <UsersIcon className="h-12 w-12 text-[var(--color-primary)] mb-3" />,
            value: "+1000",
            label: "ATENDIMENTOS",
        ...
        {
            icon: <CertificateIcon className="h-12 w-12 text-[var(--color-primary)] mb-3" />,
            value: "+10",
            label: "CERTIFICAÇÕES",
        ...
        {
            icon: <BriefcaseIcon className="h-12 w-12 text-[var(--color-primary)] mb-3" />,
            ...
            <div className="container mx-auto px-6">
                <div className="flex flex-col md:flex-row items-center justify-center gap-12 md:gap-20">
                    ...
                    <div className="md:w-2/3 text-center md:text-left relative">
                        ...
                        <p className="text-lg text-[var(--color-text)] leading-relaxed max-w-3xl mb-12">
                        ...
                        <div className="grid grid-cols-3 gap-2 sm:gap-8 text-center border-t border-[var(--color-border)] pt-8">
                          {stats.map((stat, index) => (
                              ...
                              >
                                  <div className="flex flex-col items-center p-2 sm:p-4 rounded-lg transition-transform duration-300 group-hover:scale-110">
                                      {React.cloneElement(stat.icon, { className: "h-8 w-8 sm:h-12 sm:w-12 text-[var(--color-primary)] mb-2 sm:mb-3" })}
                                      <span className="text-lg sm:text-2xl font-bold text-[var(--color-text-strong)] leading-tight">{stat.value}</span>
                                      <span className="text-[var(--color-text-subtle)] text-xs sm:text-base font-semibold tracking-wider">{stat.label}</span>
                                  </div>
                              </button>
                          ))}
                        </div>
