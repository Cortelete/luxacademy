...
const CourseCard: React.FC<CourseCardProps> = ({ course, featureCategories, onCtaClick }) => {
    ...
    return (
        <div 
            ...
            className={`cursor-pointer relative flex flex-col bg-white rounded-lg p-8 h-full transition-all duration-300 ${isHighlighted ? 'shadow-2xl ring-1 ring-[var(--color-primary)]/20 transform md:-translate-y-4' : 'shadow-lg hover:shadow-xl border border-[var(--color-border)]'}`}
        >
            {course.highlight && (
            ...
            <div className="text-center mb-8 pt-4 md:mb-8 md:pt-4 mb-2 pt-2">
                <h3 className="font-playfair font-bold text-xl md:text-2xl text-[var(--color-text-strong)] mb-2">{course.title}</h3>
                <p className="text-[var(--color-text-subtle)] text-xs md:text-sm md:min-h-[2.5rem]">{course.uniqueDescription}</p>
            </div>
            ...
            <a 
                ...
                className={`hidden md:flex w-full py-3 px-4 rounded-sm font-bold text-sm uppercase tracking-widest transition-all duration-300 items-center justify-center gap-2 ${isVip ? 'bg-[var(--color-text-strong)] text-white hover:bg-[var(--color-primary)]' : 'bg-[var(--color-primary)] text-white hover:bg-[var(--color-text-strong)]'}`}
            >
            ...
            <button 
                ...
                className={`md:hidden mt-4 w-full py-2 px-4 rounded-sm font-bold text-xs uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 ${isVip ? 'bg-[var(--color-text-strong)] text-white' : 'bg-[var(--color-primary)] text-white'}`}
            >
                Ver Detalhes
            </button>
        </div>
