...
const Hero = forwardRef<HTMLElement, HeroProps>(({ scrollToSection }, ref) => {
    ...
    return (
        ...
        >
            ...
            >
                <h1 
                    className="text-5xl sm:text-6xl md:text-8xl lg:text-9xl font-great-vibes pt-4 hero-title-golden cursor-pointer leading-snug md:leading-normal"
                >
                   {titleText}
                </h1>
                <h2 className="mt-4 text-2xl sm:text-3xl md:text-4xl font-playfair font-bold hero-subtitle uppercase">
                    Conquiste sua Independência.
                </h2>
                <button
                    onClick={scrollToSection}
                    className="mt-12 px-10 py-4 bg-[var(--color-primary)] text-[var(--color-primary-contrast)] font-bold text-lg rounded-full shadow-lg hover:bg-[var(--color-primary)]/80 transform hover:scale-105 transition-all duration-300 ease-in-out"
                >
                    Descubra os Cursos
                </button>
            </div>
